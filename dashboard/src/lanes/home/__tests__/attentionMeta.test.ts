...
describe("ATTENTION_KIND_META census", () => {
  ...
  it("has exactly the seven known AttentionRow kinds, no more, no less", () => {
   ...
   */
  it.each(KNOWN_KINDS)("%s routes to a lane the process model declares", (kind) => {
    expect(LANES).toContain(ATTENTION_KIND_META[kind].anchor.lane);
  });

  it.each(KNOWN_KINDS)("%s routes to a stage that lane actually has", (kind) => {
