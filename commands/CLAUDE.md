...
- **Never instruct raw `git` against the vault.** Use the deterministic tools (`branches action=promote`, `compile promote`, `knotica doctor repair`) so the operation stays flock-guarded and auditable. `git restore .` in particular is forbidden — it discards a user's uncommitted work.
