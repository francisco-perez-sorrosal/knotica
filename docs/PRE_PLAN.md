...
- **Safety net — doctor activation model (layered):** doctor runs only *deterministic* mechanical checks (config sanity, schema resolution, reserved names, broken links, git state incl. dirty-tree rollback offer and **unpushed-commits warning**, MCP registration). Layers: (a) on-demand `knotica doctor` / `/knotica:doctor`; (b) SessionStart hook `--quick`; (c) **harness guard** — automatically pre/post every DSPy/SIA run from Phase 3 on (a loop never starts from or leaves a broken vault). **Loop lifecycle (dec-draft-64a38a63):** originally, "no periodic daemon in MVP" (vault mutations bounded to sessions; SessionStart checked vault health). This stance is now superseded: `knotica service install` registers a lifecycle-managed loop service with the OS service manager (launchd/systemctl). The **operational** constraint relaxes (daemon auto-spawns, user never starts auxiliary processes), but the semantic invariants are preserved: client-as-brain (daemon is a headless loop, already-permitted per dec-014), stateless MCP server (the service manager controls the *daemon*, not the MCP server), clone-not-live-vault loops, flock-guarding (now serializing daemon + MCP gate + user edits), and heartbeat liveness. Reversal trigger: if one-click service management breaks in production (uninstall incomplete, state leaks, conflicts with user processes), re-adopt the session-based cadence. *Semantic* checks (contradictions, staleness) are `/knotica:lint` — LLM-driven, client-run; scheduled agent-mode later.
