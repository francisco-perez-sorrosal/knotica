# Knotica — LLM-Wiki MVP: Converged Design (v6)
...
- **`uv` is a hard prerequisite** the plugin cannot install: SessionStart hook checks `command -v uvx` and prints install instructions if absent. First `uvx --from` launch resolves the Python env (one-time latency) — setup/doctor mention it.
...
`uv tool install` from the same repo → `knotica init` interactive wizard (`--yes` for defaults) does everything `/knotica:setup` does, incl. Claude Code `claude mcp add` and Desktop config patch. Same engine, two entry points.
...
- **Plugin command aliases (ergonomics, Claude Code only):** thin `commands/*.md` shims providing the clean names `/knotica:ingest|query|lint|curate|setup|doctor|status|migrate`; each injects the same vault-resolved prompt body via the CLI.
...
**Status: pre-plan v5 persisted (2026-07-03). Next action awaits the user's go.**

**Kickoff (when the user says go):** `git init` code repo; launch **Phases 0–1 as one Standard-tier praxion pipeline** (worktree isolation, TASK_BRIEF, SDD spec with REQ IDs, three-document model), with `docs/PRE_PLAN.md` seeding the researcher/architect.
