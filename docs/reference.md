...
| `loop` | `improve action=loop` + `loop_action=` | `scalar`, `policy=""`, `mode="best"`, `eval_min_interval_hours`, `eval_window`, `eval_num_threads`, `arena_scorer`, `confirm=""`, `num_threads` | Actions: `run_once`, `set_baseline`, `baseline_policy`, `rebaseline`, `cadence`, `run_eval`. No (nonce-confirmed instead — see below). |
