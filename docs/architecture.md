...
- Lane rail state (pure, framework-free derivation — no Preact, no DOM, no fetch) →
  ...
  the client's own LLM as a slash command.
- Lane components → one directory per lane under `dashboard/src/lanes/`: `learn/`, `answer/`,
  `fill/`, `improve/`, `tend/`. Presentation two lanes share sits beside the tree
