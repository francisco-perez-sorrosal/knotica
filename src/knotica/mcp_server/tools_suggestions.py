"""Suggestion-queue MCP tools -- ``suggestions_read`` + ``suggestions_review``.

Two deterministic tools over the committed human-approval queue at
``<topic>/.knotica/suggestions/suggestions.jsonl``: one **read** tool that pages
it for the dashboard/interactive client, and one **action-parameterized
mutating** tool that flips one suggestion's lifecycle status after a human
decision. Both are stateless, topic-explicit, and honor the ``NOT_CONFIGURED``
contract via :func:`with_resolved_vault`.

The queue *upstream* of this one -- ``gaps.jsonl``, plus the discovery drain
between them -- lives in ``tools_gaps``. One module per queue.
...
:func:`knotica.core.gapfill.apply_decision`, which does not pull the heavy search
chain. The read tool re-uses the opaque search cursor (base64 offset token) so a
stale/malformed cursor fails closed as ``INVALID_CURSOR`` exactly like the search
surface (dec-002).
"""
