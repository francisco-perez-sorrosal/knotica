...
date: 2026-07-03
summary: Expose ~10 thin deterministic tools (no progressive disclosure), consolidate list_links/backlinks into one direction-parameterized tool, and make write_page a single transactional fat-write (scrub+write+commit+log).
...
3. **`write_page` is a transactional fat-write** — secret-scrub + atomic write + one git commit + log append execute as one indivisible unit. These side effects *always* co-occur; splitting them would let the client leave the vault half-committed (page written but not logged, or committed but not scrubbed). This is the justified fat-tool: steps always occur together and combined semantics are clear.
...
- Positive: a 10-tool surface a model selects over reliably; vault invariants (one-commit-per-op, scrub, log) enforced *inside* the tool, not delegated to model discipline; room to grow before progressive disclosure is warranted.
- Negative: if the tool count grows past ~20 in later phases (wiki_query, compile triggers, SIA moves), progressive disclosure must be revisited then.
