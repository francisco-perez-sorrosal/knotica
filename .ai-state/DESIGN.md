...
- **dec-draft-315e275a** — Four-way fault classifier + heal-redirect: an ordered first-match cascade over
...
- **dec-draft-d777755b** — Gap-record schema v1: knowledge-cause verdicts persist as a new
...
- **dec-draft-f4584c2f** — Discovery contract: a `runtime_checkable` `SearchProvider` protocol produces a
...
- **dec-draft-c7d82c89** — Discovery HTTP boundary: **direct `httpx` REST** behind one thin shared client for all
