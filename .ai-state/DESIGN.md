...
| `guillotine/` | Pure read-only analysis layer; its transaction-bearing adapter is `core/operations/guillotine.py` | **No** by design. One unguarded exception: the zero-caller `report.py::write_artifacts` (see the row above) sits outside every current scan |
...
- **`dec-draft-c20759d6`** — Package-level granularity for the four un-modeled trees, plus a
  ...
  re-labelled an explicit residual. Realizes reversal trigger (a) of `dec-draft-4b91f4f7`
