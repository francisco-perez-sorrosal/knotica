...
| `src/knotica/mcp_server/` | FastMCP adapter: **21 registered tools** — 13 Tier-1 flat conversational tools, `open_dashboard`, the unlaned `vault` dispatcher, and the 6 process-lane dispatchers (`tools_dispatch_<lane>.py`, generated from `core/process_model.py` by `tools_dispatch_lane_common.py`) — plus 4 resources + 1 UI resource, 4 prompts. The operator-tier verbs — the eight absorbed topical dispatchers (`arena`, `branches`, `compile`, `datasets`, `golden`, `loop`, `notes`, `vault_health`) and the flat verbs the lanes took over (`create_topic`, `lint_check`, `metrics_read`, `baseline_probe`, `prompt_diff`, `suggestions_read`, `suggestions_review`, `gaps_read`, `gapfill_discover`, `review_gap`, `source_ingest_open`, `source_ingest_submit`, `session_status`, `ingest_activity_read`) — publish **no tool name of their own**: their `register_*_lane_tools` functions run only against `tools_dispatch_lane_common.py`'s `_HandlerCapture`, which keeps the function object and discards the schema. That capture is the handler seam, not an alias layer; the old flat names return an unknown-tool error (`dec-050`). `vault_ctx.with_resolved_vault` is the per-call config-resolution and error-mapping seam every tool routes through — the concrete form of the stateless-server invariant. Two further single-declaration seams sit beside it: `tool_params.py` holds one grounded `Annotated` alias per parameter *meaning*, so a lane's union call shape inherits each parameter's schema description and advisory enum with no second declaration to drift; `lane_next.py` projects the success envelope's `next_stage` block from `core/process_model.py`, the server-side twin of the dashboard's `dec-106` Phase 6. Named `mcp_server` to avoid shadowing the `mcp` SDK (`dec-009`) | Built |
...
| **MCP tool surface** | Two-tier topology: 13 Tier-1 flat conversational tools carry the high-density verbs the client calls mid-turn; the 6 process lanes route the operator long tail, their action tables and call shapes generated from `core/process_model.py` so a verb's lane, rail position and call shape cannot disagree with the declaration. `vault` and `open_dashboard` stay flat and unlaned. 21 registrations; the absorbed verbs keep their `@mcp.tool` registration solely as the lane dispatchers' handler seam, with no alias layer. `dec-041`, `dec-045`, `dec-050` | `mcp_server/` | Built |
