...
| **MCP tool surface** | Two-tier topology: 25 flat conversational tools carry the high-density verbs; action-parameterized dispatchers route the operator long tail — the 9 topical ones, plus the 6 process lanes whose action tables and call shapes are generated from `core/process_model.py` so a verb's lane, rail position and call shape cannot disagree with the declaration. `dec-041`, `dec-045`, `dec-050` | `mcp_server/` | Built |
