...
- **Writing the REQ-TOOL-05 assertion surfaced a new defect rather than confirming a clean one.**
  All four mutating tools refused an empty topic, but in three different shapes: two returned the
  typed `{"error": {code, message, fix, retryable}}` envelope while `write_page` and `store_source`
  returned a raw exception string, because the write path had no counterpart to the read side's
  `map_read_exception` and anything an operation *raised* escaped the adapter. Ledgered as `td-041`
  rather than frozen into this matrix, then fixed: `envelope.map_write_exception` now types the
  argument-validation class as `INVALID_ARGUMENT` and re-raises anything it cannot type, so a
  genuine bug stays a crash instead of becoming advice.
