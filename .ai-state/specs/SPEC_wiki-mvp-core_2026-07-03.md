...
| **REQ-TOOL-02** reads produce zero commits and never take the write lock | `tests/test_mcp_read.py::test_reads_make_zero_git_commits`; `tests/test_links.py::test_link_queries_never_commit_or_dirty_the_vault`; `tests/test_lint.py::test_lint_makes_no_commit_and_leaves_a_clean_tree`; `tests/test_mcp_status.py::test_wiki_status_is_read_only`; `tests/test_architecture_boundaries.py::test_adapters_do_not_import_the_vault_lock` | `mcp_server/tools_read.py`, `core/lint.py`, `core/links.py`, `search/` | PARTIAL — the zero-commit clause is asserted on every read path; the **"never acquires the write lock"** clause is asserted only *structurally for adapters* (adapters cannot import `core.lock`). No behavioral test proves a `core` read path (`lint`, `links`, `search`) leaves the flock untaken |
...
| **REQ-REC-03** root `SCHEMA.md` documents all five record formats under one `schema_version:` | `tests/test_schema.py::test_root_constitution_reads_with_its_version_and_frontmatter_stripped_body`, `::test_effective_schema_version_is_the_root_constitutions`; `tests/test_lint.py::test_missing_root_schema_version_is_flagged`, `::test_overlay_schema_version_conflict_is_flagged`; `tests/test_template.py::test_template_instantiates_with_the_full_root_and_topic_inventory` | `vault-template/SCHEMA.md`, `core/schema.py`, `core/lint.py` | PARTIAL — the single versioned constitution is asserted (presence, version, overlay precedence, conflict linting); **no test asserts that root `SCHEMA.md` actually documents the five record formats.** `tests/test_records.py` names `SCHEMA.md §Machine-record schemas` as its contract source *in prose only* — the frozen grammars are re-declared in `tests/support/vault.py`, never read back from the file, so the doc and the code can silently diverge |
...
| **REQ-PROMPT-02** prompt body satisfies the INTERFACE_DESIGN §2.3 checklist | `tests/test_prompts.py::test_operation_bodies_embed_the_topic_inference_policy_verbatim`, `::test_flywheel_operations_solicit_curation`, `::test_operation_bodies_point_at_the_resolved_schema_resource`; `tests/test_mcp_prompts.py::test_shipped_prompts_reference_tools`, `::test_every_prompt_tool_reference_is_a_registered_tool` | `vault-template/.knotica/prompts/`, `core/prompts.py` | PARTIAL — verbatim topic-inference block, curation solicitation, resolved-schema pointer, and exact tool names are all asserted; **citation discipline on `query` has no assertion** (the only `citation` matches in the prompt tests are the substring inside *soli-citation* and the `citation_key` tool argument), and the `read → act → update index → log` **ordering** is not asserted as a sequence |
...
| **REQ-PLUGIN-01** `[drill]` `.mcp.json` launches via `uvx --from ${CLAUDE_PLUGIN_ROOT} knotica mcp`, never `alwaysLoad` | *none* | `.mcp.json` | UNTESTED — the shipped `.mcp.json` still matches the REQ verbatim (verified by inspection 2026-08-04: `uvx --from ${CLAUDE_PLUGIN_ROOT} knotica mcp`, no `alwaysLoad` key), but **nothing asserts it**; a manifest edit would go undetected. Cheapest gap on this list to close |
| **REQ-PLUGIN-02** `[drill]` backgrounded, idempotent SessionStart pre-warm | `tests/test_hooks_session_start.py::test_warm_path_makes_exactly_one_status_nudge_call_and_stays_fast` | `hooks/session_start.sh`, `hooks/hooks.json` | PARTIAL — the non-blocking/fast-path property is asserted; **no test asserts the pre-warm command itself** (`uvx --from "${CLAUDE_PLUGIN_ROOT}" knotica --version`), that it is backgrounded, or that it is idempotent |
...
### Coverage summary (as re-derived 2026-08-04)

| Status | Count | Requirements |
|---|---|---|
| `COVERED` | 31 | MUT-01…05, CFG-01…04, TOOL-01, TOOL-03, TOOL-04, TOOL-06, TOOL-07, ERR-01, ERR-02, SRCH-01, REC-01, REC-02, PROMPT-01, PROMPT-03, RES-01, PLUGIN-03, CLI-01…05, VLT-01…03 |
| `PARTIAL` | 7 | TOOL-02, TOOL-05, REC-03, PROMPT-02, PLUGIN-02, CLI-06, CLI-07 |
| `UNTESTED` | 1 | PLUGIN-01 |
